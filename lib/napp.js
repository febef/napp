...
napp.prototype._loadComponents = function(){
  ...
  fs
    ...
    .forEach( file => {
      ...
      this.components[file.slice(0, -3)] = require(path.join(this.config.componentsPath, file));
    });
...
napp.prototype._runComponents = function(){
  ...
  for (i=0; i<this.runOrder.length; i++){
    ...
    this.components[this.runOrder[i]].runed = true;
...
napp.prototype.stop = function(component){
  ...
  var i;
  if (!this.components[component].runed)
    return false;
  ...
  this.components[component].runed = false;
